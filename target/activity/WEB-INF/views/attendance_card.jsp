            ...
            <form id="form" enctype="multipart/form-data" class="form-horizontal">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <div class="col-md-2">
                            <label for="attendanceCardName">游园卡模板名称</label>
                        </div>
                        <div class="col-md-10">
                            <input type="text" class="form-control" id="attendanceCardName" name="attendanceCardName" placeholder="请输入游园卡模板名称">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-md-2">
                            <label for="gardenCardTemplate">游园卡模板</label>
                        </div>
                        <div class="col-md-10">
                            <input type="file" class="file-loading" id="gardenCardTemplate" name="multipartFile" placeholder="请选择游园卡模板">
                        </div>
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form id="form1" enctype="multipart/form-data" class="form-horizontal">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                    <h4 class="modal-title" id="myModalLabel1">游园卡模板修改</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="schemeId" name="schemeId">
                    <div class="form-group">
                        <div class="col-md-2">
                            <label for="attendanceCardName1">游园卡模板名称</label>
                        </div>
                        <div class="col-md-10">
                            <input type="text" class="form-control" id="attendanceCardName1" name="attendanceCardName" placeholder="请输入游园卡模板名称">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-md-2">
                            <label for="gardenCardTemplate1">游园卡模板</label>
                        </div>
                        <div class="col-md-10">
                            <input type="file" class="file-loading" id="gardenCardTemplate1" name="multipartFile" placeholder="请选择游园卡模板">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>关闭</button>
...
<script>
    ...
    $("#gardenCardTemplate").fileinput(options);
    $("#gardenCardTemplate1").fileinput(options);
    //加载表格,由于bootstrap的刷新也在这,所以先销毁一下表格在加载表示刷新
    $("#table").bootstrapTable('destroy');
    $('#table').bootstrapTable({
        url: basePath + 'attendanceCard/listAll.do',                      //请求后台的URL（*）
        method: 'GET',                      //请求方式（*）
        dataType: "json",
        pagination: true,
        search: true,
        ...
        columns: [{
            ...
            field: 'attendanceCardName',
            title: '游园卡名称',
            visible:true,
            align: 'center'
        },{
            field: 'gardenCardTemplate',
            title: '游园卡模板',
            visible:true,
            align: 'center',
         ...
         $("#btn_submit").on("click",function(){
             if(validateForm().form()){
                 $.ajax({
                     ...
                     cache: false,
                     dataType:"json",
                     url: basePath+"attendanceCard/add.do",
     ...
     function update() {
        ...
        if (a.length <= 0) {
            ...
            $("#btn_update").on("click",function(){
                $.ajax({
                    url: basePath + "attendanceCard/update.do",
                    type:"post",
                    dataType:"json",
